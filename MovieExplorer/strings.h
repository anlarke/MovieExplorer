...
#define IDS_AUTOCATEGORIES 1040
#define IDS_DATABASE 1041
#define IDS_INDEXEXTENSIONS 1042
#define IDS_INDEXDIRECTORIES 1043
#define IDS_ONLYUSE 1044
#define IDS_MAXINFOAGE 1045
#define IDS_INFOSERVICE 1046
#define IDS_TITLE 1047
#define IDS_YEAR 1048
#define IDS_STORYLINE 1049
#define IDS_POSTER 1050
#define IDS_RATING 1051
#define IDS_COMBINED 1052
#define IDS_NONE 1053
#define IDS_CATEGORIES 1054
#define IDS_DIRECTORIES 1055
#define IDS_FOLDERORFILE 1056
#define IDS_ONLYAVAILABLEON 1057
#define IDS_YOUCANUSETXT 1058
#define IDS_EDITMOVIE 1059
#define IDS_IDENTIFICATION 1060
#define IDS_INFORMATION 1061
#define IDS_FILENAME 1062
#define IDS_FILESIZE 1063
#define IDS_OPTIONALPROP 1064
#define IDS_SEENMOVIE 1065
#define IDS_HIDEMOVIE 1066
#define IDS_SURETOHIDE 1067
#define IDS_SURETODELETEFILE 1068
#define IDS_SURETODELETEDIRECTORY 1069
#define IDS_FILENOTFOUND 1070
#define IDS_READY 1071
#define IDS_LOADINGDATABASE 1072
#define IDS_UPDATINGDATABASE 1073
#define IDS_SYNCINGDATABASE 1074
#define IDS_TOOLTIP_PLAY 1075
#define IDS_TOOLTIP_DIR 1076
#define IDS_TOOLTIP_SEEN 1077
#define IDS_TOOLTIP_EDIT 1078
#define IDS_TOOLTIP_HIDE 1079
#define IDS_TOOLTIP_DELETE 1080
#define IDS_TOOLTIP_TOOLS 1081
#define IDS_TOOLTIP_SEENMAIN 1082
#define IDS_TOOLTIP_SORT 1083
#define IDS_TOOLTIP_VIEW 1084
#define ID_SORT_TITLEASC 1085
#define ID_SORT_TITLEDES 1086
#define ID_SORT_YEARASC 1087
#define ID_SORT_YEARDES 1088
#define ID_SORT_RATINGASC 1089
#define ID_SORT_RATINGDES 1090
#define ID_SORT_FILETIMEASC 1091
#define ID_SORT_FILETIMEDES 1092
...
inline void LoadStrings()
{
	...
	GetLangMgr()->SetString(nLanguage, 40, _T("Create categories for movies and tv"), false);
	GetLangMgr()->SetString(nLanguage, 41, _T("Database"), false);
	GetLangMgr()->SetString(nLanguage, 42, _T("Index files with the following extensions"), false);
	GetLangMgr()->SetString(nLanguage, 43, _T("Include directories in list"), false);
	GetLangMgr()->SetString(nLanguage, 44, _T("Only use"), false);
	GetLangMgr()->SetString(nLanguage, 45, _T("Maximum movie information age (weeks)"), false);
	GetLangMgr()->SetString(nLanguage, 46, _T("Information service"), false);
	GetLangMgr()->SetString(nLanguage, 47, _T("Title"), false);
	GetLangMgr()->SetString(nLanguage, 48, _T("Year"), false);
	GetLangMgr()->SetString(nLanguage, 49, _T("Storyline"), false);
	GetLangMgr()->SetString(nLanguage, 50, _T("Poster"), false);
	GetLangMgr()->SetString(nLanguage, 51, _T("Rating"), false);
	GetLangMgr()->SetString(nLanguage, 52, _T("Combined"), false);
	GetLangMgr()->SetString(nLanguage, 53, _T("None"), false);
	GetLangMgr()->SetString(nLanguage, 54, _T("Categories"), false);
	GetLangMgr()->SetString(nLanguage, 55, _T("Directories"), false);
	GetLangMgr()->SetString(nLanguage, 56, _T("Folder/File"), false);
	GetLangMgr()->SetString(nLanguage, 57, _T("Only available on"), false);
	GetLangMgr()->SetString(nLanguage, 58, _T("Tip: You can also specify plain text files (*.txt) containing a list of movies separated by line breaks."), false);
	GetLangMgr()->SetString(nLanguage, 59, _T("Edit Movie"), false);
	GetLangMgr()->SetString(nLanguage, 60, _T("Identification"), false);
	GetLangMgr()->SetString(nLanguage, 61, _T("Information"), false);
	GetLangMgr()->SetString(nLanguage, 62, _T("File name"), false);
	GetLangMgr()->SetString(nLanguage, 63, _T("File size"), false);
	GetLangMgr()->SetString(nLanguage, 64, _T("Optional properties"), false);
	GetLangMgr()->SetString(nLanguage, 65, _T("I have seen this movie"), false);
	GetLangMgr()->SetString(nLanguage, 66, _T("Hide this movie"), false);
	GetLangMgr()->SetString(nLanguage, 67, _T("Are you sure you want to hide this movie?"), false);
	GetLangMgr()->SetString(nLanguage, 68, _T("Are you sure you want to permanently delete this file from the computer?"), false);
	GetLangMgr()->SetString(nLanguage, 69, _T("Are you sure you want to permanetly delete this directory from the computer?"), false);
	GetLangMgr()->SetString(nLanguage, 70, _T("File not found"), false);
	GetLangMgr()->SetString(nLanguage, 71, _T("Ready"), false);
	GetLangMgr()->SetString(nLanguage, 72, _T("Loading database"), false);
	GetLangMgr()->SetString(nLanguage, 73, _T("Collecting and updating movie information"), false);
	GetLangMgr()->SetString(nLanguage, 74, _T("Synchronizing with files on disk"), false);
	GetLangMgr()->SetString(nLanguage, 75, _T("Play"), false);
	GetLangMgr()->SetString(nLanguage, 76, _T("Directory"), false);
	GetLangMgr()->SetString(nLanguage, 77, _T("Watched"), false);
	GetLangMgr()->SetString(nLanguage, 78, _T("Edit"), false);
	GetLangMgr()->SetString(nLanguage, 79, _T("Hide"), false);
	GetLangMgr()->SetString(nLanguage, 80, _T("Delete"), false);
	GetLangMgr()->SetString(nLanguage, 81, _T("Preferences"), false);
	...
	GetLangMgr()->SetString(nLanguage, 44, _T("Gebruik alleen"), false);
	GetLangMgr()->SetString(nLanguage, 45, _T("Maximale filminformatie leeftijd (weken)"), false);
	GetLangMgr()->SetString(nLanguage, 46, _T("Informatie service"), false);
	GetLangMgr()->SetString(nLanguage, 47, _T("Titel"), false);
	GetLangMgr()->SetString(nLanguage, 48, _T("Jaar"), false);
	GetLangMgr()->SetString(nLanguage, 49, _T("Plot"), false);
	GetLangMgr()->SetString(nLanguage, 50, _T("Poster"), false);
	...
	GetLangMgr()->SetString(nLanguage, 58, _T("Tip: U kunt ook platte tekstbestanden (*.txt) met daarin een lijst van films gescheiden door regeleinden opgeven."), false);
	GetLangMgr()->SetString(nLanguage, 59, _T("Wijzig Film"), false);
	GetLangMgr()->SetString(nLanguage, 60, _T("Identificatie"), false);
	GetLangMgr()->SetString(nLanguage, 61, _T("Informatie"), false);
	GetLangMgr()->SetString(nLanguage, 62, _T("Bestandsnaam"), false);
	GetLangMgr()->SetString(nLanguage, 63, _T("Bestandsgrootte"), false);
	GetLangMgr()->SetString(nLanguage, 64, _T("Optionele eigenschappen"), false);
	GetLangMgr()->SetString(nLanguage, 65, _T("Ik heb deze film gezien"), false);
	GetLangMgr()->SetString(nLanguage, 66, _T("Verberg deze film"), false);
	GetLangMgr()->SetString(nLanguage, 67, _T("Weet u zeker dat u deze film wilt verbergen?"), false);
	GetLangMgr()->SetString(nLanguage, 68, _T("Weet u zeker dat u dit bestand permanent van de computer wilt verwijderen?"), false);
	GetLangMgr()->SetString(nLanguage, 69, _T("Weet u zeker dat u deze map permanent van de computer wilt verwijderen?"), false);
	GetLangMgr()->SetString(nLanguage, 70, _T("Bestand niet gevonden"), false);
	GetLangMgr()->SetString(nLanguage, 71, _T("Klaar"), false);
	GetLangMgr()->SetString(nLanguage, 72, _T("Laden van database"), false);
	GetLangMgr()->SetString(nLanguage, 73, _T("Bezig met opzoeken en updaten van film informatie"), false);
	GetLangMgr()->SetString(nLanguage, 74, _T("Synchroniseren met de bestanden op de schijf"), false);
	GetLangMgr()->SetString(nLanguage, 75, _T("Afspelen"), false);
	GetLangMgr()->SetString(nLanguage, 76, _T("Map openen"), false);
	GetLangMgr()->SetString(nLanguage, 77, _T("Gezien"), false);
	GetLangMgr()->SetString(nLanguage, 78, _T("Bewerken"), false);
	GetLangMgr()->SetString(nLanguage, 79, _T("Verbergen"), false);
	GetLangMgr()->SetString(nLanguage, 80, _T("Verwijderen"), false);
	GetLangMgr()->SetString(nLanguage, 81, _T("Voorkeuren"), false);
